...
export default async function handler(nodeReq, nodeRes) {
  ...
  if (req.method === "PATCH") {
    ...
    if (error) return res.json({ error: "Couldn't update this plan." }, 500);
    return res.json(data);
  }

  if (req.method === "DELETE") {
    const { error } = await supabase.from("content_plans")
      .delete().eq("id", id).eq("user_id", user.id);
    if (error) return res.json({ error: "Couldn't delete this plan." }, 500);
